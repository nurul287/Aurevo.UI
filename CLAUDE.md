...
**AI monitoring dashboard** (`src/pages/admin/admin-ai-metrics-page.tsx`, route `/admin/ai`) — admin view of the chatbot's usage/latency/cost/retrieval metrics via `useAiMetrics(days)` → `GET /admin/ai-metrics`. Follows the `admin-dashboard-page` pattern (stat cards, Skeleton, error Card + Retry) plus a **recharts** daily-volume bar chart and a tool-usage/retrieval panel. The chart is themed with CSS custom properties (`var(--color-*)`) so it tracks light/dark, and wrapped in `ResponsiveContainer` for mobile. recharts is the only charting dep. The backing `chat_metrics` table + endpoint live in Aurevo.BE.
...
- **Mandatory**: after any FE or BE code change (this repo or the sibling `Aurevo.BE`) — new/edited/deleted files, not docs-only edits — run `graphify update .` in the changed repo before ending the task, so the graph never drifts stale. AST-only, no API cost, so there's no reason to skip it.
