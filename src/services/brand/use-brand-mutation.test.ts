...
describe("brand mutations", () => {
  ...
  it("useCreateBrand posts multipart form data and shows a success toast", async () => {
    ...
    expect(showSuccess).toHaveBeenCalledWith("Brand Created", "Brand has been successfully created");
  });
  ...
  it("useUpdateBrand shows an error toast on failure", async () => {
    ...
    expect(showError).toHaveBeenCalledWith("Failed to Update Brand", "Slug already exists");
  });

  it("useDeleteBrand deletes and shows a success toast", async () => {
    server.use(
      http.delete(`${API_URL}/brands/b1`, () => new HttpResponse(null, { status: 204 }))
    );
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(showSuccess).toHaveBeenCalledWith("Brand Deleted", "Brand has been successfully deleted");
  });
  ...
  it("useBulkUpdateBrandStatus activates every brand and reports the count", async () => {
    ...
    const { result } = renderHookWithQueryClient(() => useBulkUpdateBrandStatus());
    result.current.mutate({ brandIds: ["b1", "b2", "b3"], isActive: true });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(showSuccess).toHaveBeenCalledWith("Brands Updated", "3 brands have been activated");
  });
});
