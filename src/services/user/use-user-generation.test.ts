...
describe("useCreateUserFromCheckout", () => {
  ...
  it(
    "signs up the user and creates a BE profile",
    async () => {
      ...
      const { result } = renderHookWithQueryClient(() => useCreateUserFromCheckout());
...
describe("useMigrateGuestCartToNewUser", () => {
  it("posts the guest session id to /cart/migrate", async () => {
    ...
    const { result } = renderHookWithQueryClient(() => useMigrateGuestCartToNewUser());
