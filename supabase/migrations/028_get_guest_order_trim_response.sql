-- Trim get_guest_order response: omit guest_token and other sensitive columns.
...
COMMENT ON FUNCTION public.get_guest_order(uuid, text) IS
  'Returns order confirmation fields + line items when guest_token matches (no guest_token in response).';
